...
function Handle(delay_ms=60*1000){
    ...
    this.buildUrl = (shop_domain, page=1, limit=50, created_at_min=this.getdateString()) => 
        `https://${shop_domain}/api/3.0/orders.json?page=${page}&limit=${limit}&created_at_min=${created_at_min}`;
    ...
    this.limit = 50;
    this.timestamp = new Date("1900-01-01 00:00:00");
    //  first time load all the order list (created datetime default = "1900-01-01 00:00:00")
    this.url = this.buildUrl(this.shop_domain, 1, this.limit, this.dateString(this.timestamp));
    ...
    this.getData = async(is_filter=false, current_page=1, page_length=-1) => {
        ...
        this.url = this.buildUrl(this.shop_domain, current_page, this.limit, this.dateString(this.timestamp));
        this.config = this.buildConfig(this.url, this.access_token);
